    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        for i, agent_info in enumerate(discovered):
            ...
            remote_agent = RemoteA2aAgent(
                ...
                httpx_client=create_authenticated_client(agent_well_known, timeout=httpx.Timeout(120.0, connect=10.0))
            )
    ...
    sub_agents=[
        ...
        ConditionalRemoteAgent(
            ...
            remote_agent=RemoteA2aAgent(
                ...
                httpx_client=create_authenticated_client(os.environ.get("VIZ_AGENT_CARD_URL", "http://localhost:8006/a2a/agent/.well-known/agent-card.json"), timeout=httpx.Timeout(120.0, connect=10.0))
            )
        ),
        ...
        RemoteA2aAgent(
            ...
            httpx_client=create_authenticated_client(os.environ.get("SYNTHESIZER_AGENT_CARD_URL", "http://localhost:8004/a2a/agent/.well-known/agent-card.json"), timeout=httpx.Timeout(120.0, connect=10.0))
        )
    ]
)
